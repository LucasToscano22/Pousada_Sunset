...
def handle_client(conn, addr):
    ...
    while True:
        try:
            ...
            reserva = message_info['reserva']

            print("Dados da reserva recebidos:")
            print(f"Nome: {reserva['nome']}")
            print(f"E-mail: {reserva['email']}")
            print(f"Quarto: {reserva['quarto']}")
            print(f"Pagamento: {reserva['pagamento']}")
            ...
            formato_data = "%d/%m/%Y"
            data_inicio = datetime.strptime(message_info['data_inicio'], formato_data)
            data_fim = datetime.strptime(message_info['data_fim'], formato_data)

            # Verificar se as datas estão disponíveis
            if verificar_disponibilidade(reserva['quarto'], data_inicio, data_fim):
                adicionar_reserva(reserva['quarto'], data_inicio, data_fim)
